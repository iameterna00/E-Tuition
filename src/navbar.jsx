...
const Navbar = () => {
  ...
  const [isScrolled, setIsScrolled] = useState(false); // New state to track scroll position
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > 50) {  // Set the scroll position where you want the navbar to change
        setIsScrolled(true);
      } else {
        setIsScrolled(false);
      }
    ...
    <nav className={`navbar ${isScrolled ? 'solid' : ''}`}>
      <div className="navbarcontents">
        ...
        <div className={`navlinks ${menuOpen ? 'active' : ''}`}>
          ...
          <Link style={{textDecoration:'none', color:'inherit'}} to={'/iamatuitor'}>
            <button className="navbuttons">Become a Tutor</button>
          </Link>
          <Link style={{textDecoration:'none', color:'inherit'}} to={'/onlineclasses'}>
